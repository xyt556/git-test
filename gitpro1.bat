    ...
    if /i "!del_confirm!"=="y" (
        ...
        if errorlevel 1 (
            ...
            if /i "!force_del!"=="y" (
                git branch -D "!del_branch!"
                if errorlevel 1 goto ERR
                echo [OK] 已强制删除分支: !del_branch!
            )
        ) else (
            echo [OK] 已删除分支: !del_branch!
        )
    )
